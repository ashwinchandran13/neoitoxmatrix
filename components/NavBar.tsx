...
const NavBar = () => {
  ...
  return (
    ...
    <div className="sticky flex justify-between w-screen p-4 drop-shadow-2xl shadow-matrix_bg_color bg-matrix_green_header">
      ...
      <div className='hidden w-5/12 md:flex'>
        <div className='flex justify-between space-x-10'>
          {headers.map((header) => (
            <span className='flex items-center col-span-1 text-lg font-medium text-white' key={header.name}>
              <a href='' className='hover:border-b-4 hover:border-matrix_green_btn'>{header.name}</a>
            </span>
          ))}
        </div>
      </div>
      <div className='items-center hidden w-1/12 md:flex'>
